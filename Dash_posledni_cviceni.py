...
def update_figure(plot_type):
    first_age = titanic[titanic.pclass == 1].age
    second_age = titanic[titanic.pclass == 2].age
    third_age = titanic[titanic.pclass == 3].age

    plot_function = go.Histogram if plot_type == 'hist' else go.Box
    trace1 = plot_function(x = first_age, opacity = 0.75, name = 'První třída')
    trace2 = plot_function(x = second_age, opacity = 0.75, name = 'Druhá třída')
    trace3 = plot_function(x = third_age, opacity = 0.75, name = 'Třetí třída')
    ...
    figure={
        'data': data,
        'layout': {
            'title': 'Věk cestujících podle třídy',
        },
